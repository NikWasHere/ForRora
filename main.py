...
def is_outside_bouquet_area(x, y):
    bouquet_left = -250
    bouquet_right = 250
    bouquet_bottom = -250
    bouquet_top = 250

    if x < bouquet_left or x > bouquet_right or y < bouquet_bottom or y > bouquet_top:
        return True
    return False

def draw_background_hearts():
    t.speed(0)  
    colors = ["LightPink", "MistyRose", "LavenderBlush", "Thistle"]
    for _ in range(10):  
        while True:
            x = random.randint(-300, 300)
            y = random.randint(-300, 300)
            if is_outside_bouquet_area(x, y):
                break
...
def add_text():
    t.penup()
    t.goto(0, -250)  # Posisi teks di bawah bouquet
    t.pendown()
    t.color("DarkBlue")
    t.write(" For Latetia Aurora", align="center", font=("Arial", 24, "bold"))

add_text()

t.hideturtle()
draw_background_hearts()
turtle.done()
